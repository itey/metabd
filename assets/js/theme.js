...
function initSearch () {
  ...
  if(!document.getElementById(`search-toggle-${suffix}`)) return;
  ...
  if (isMobile) {
	const searchInputshow = document.getElementById(`search-input-mobileshow`)
	window._searchMobileOnce = true
	if(!searchInputshow) return;
	searchInputshow.addEventListener('click', () => {
		document.body.classList.add('mbsearch')
    ...
    document.getElementById('search-cancel-mobile').addEventListener('click', () => {
	  ...
	  document.body.classList.remove('mbsearch')
...
function onScroll () {
  ...
  if(document.getElementsByClassName('smheadnavbox')[0])$headers.push(document.getElementsByClassName('smheadnavbox')[0])
  ...
  function handleScrollEvent () {
    ...
    forEach($headers, $header => {
      if (scroll > ACCURACY) {
		...
		if(document.getElementsByClassName("smheadnavbox")[0]){
			document.getElementsByClassName("smheadnavbox")[0].classList.add('smheadnavboxac')
		}
	  } else if (scroll < -ACCURACY || window.newScrollTop <= 20) {
        $header.classList.remove('animate__fadeOutUp')
        animateCSS($header, ['animate__fadeInDown', 'animate__faster'], true)
		if(document.getElementsByClassName("smheadnavbox")[0]){
			document.getElementsByClassName("smheadnavbox")[0].classList.remove('smheadnavboxac')
		}
	  }
    })
